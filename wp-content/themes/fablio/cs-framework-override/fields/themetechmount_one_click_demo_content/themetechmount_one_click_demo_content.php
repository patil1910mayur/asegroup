...
class CSFramework_Option_themetechmount_one_click_demo_content extends CSFramework_Options {
  ...
  public function output(){
   ...
   wp_enqueue_style(
		'themetechmount-one-click-demo-setup',
   ...
   wp_enqueue_script(
	...
	// checking if "Fablio Demo Content Setup" plugin is installed
	if( class_exists('themetechmount_fablio_one_click_demo_setup') ){
		$button_disabled = "";
	} else {
		$button_disabled = "disabled";
		$error[]         = esc_attr__('The "Fablio Demo Content Setup" plugin is not installed or activated. This plugin is required to Import Demo Content.', 'fablio');
	}
	
	// checking if "Contact Form 7" plugin is installed
	if( !defined('WPCF7_VERSION') ){
		$button_disabled = "disabled";
		$error[]         = esc_attr__('The "Contact Form 7" plugin is not installed or activated. This plugin is required to Import Demo Content. So please install it.', 'fablio');
	}
	...
	<div class="themetechmount-one-click-demo-content-wrapper">
		...
		<div class="tm-demo-setup-success-message">
			...
			<?php esc_attr_e('Demo setup done successfully! Now you can start to edit your site', 'fablio'); ?>
		</div>
		<?php endif; ?>
		...
		<div id="import-demo-data-results-wrapper" style="display:none;">
			...
			<?php if( count($error)>0 ){ ?>
				...
				<div class="alert-info tm-one-click-error-message">
					<h4><?php esc_attr_e( 'Please correct the errors given below:', 'fablio' ) ?></h4>
					<ul>
					<?php
					foreach( $error as $line ){
						echo '<li>' . sprintf( esc_attr__('%s ', 'fablio' ) , $line ) . '</li>';
					}
					?>
					</ul>
					<a href="<?php echo esc_url( admin_url( 'themes.php?page=tgmpa-install-plugins' ) ) ?>"><?php esc_attr_e('Click here to install the plugin(s)', 'fablio' ); ?></a> &nbsp; 
					<a href="#" class="tm-one-click-error-close"><?php esc_attr_e('Close', 'fablio' ); ?></a>
				</div>
			...
			<?php } else { ?>
				...
				if( function_exists('themetechmount_fablio_one_click_html') ){
					themetechmount_fablio_one_click_html();
				}
				?>
