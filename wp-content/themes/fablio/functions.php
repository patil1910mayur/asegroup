<?php
/**
 * Fablio functions and definitions
 *
...
function themetechmount_setup() {
	 ...
	 * If you're building a theme based on fablio, use a find and replace
	 * to change 'fablio' to the name of your theme in all the template files
	 */
	load_theme_textdomain( 'fablio', get_template_directory() . '/languages' );
	...
	register_nav_menus( array( // https://codex.wordpress.org/Function_Reference/has_nav_menu
		'themetechmount-main-menu'			=> esc_html__( 'Main Menu', 'fablio' ),
	) );
	
	if( function_exists('themetechmount_fablio_cs_framework_init')){
		register_nav_menus( array( // https://codex.wordpress.org/Function_Reference/has_nav_menu
			'themetechmount-footer-menu'		=> esc_html__( 'Footer Menu', 'fablio' ),
		) );
	}
...
add_action('wp_loaded', 'fablio_prefix_output_buffer_start');
function fablio_prefix_output_buffer_start() { 
    ob_start("fablio_prefix_output_callback"); 
}
function fablio_prefix_output_callback($buffer) {
