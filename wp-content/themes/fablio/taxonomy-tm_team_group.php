...
get_header(); ?>
	...
	<section id="primary" class="content-area <?php echo themetechmount_sanitize_html_classes(themetechmount_sidebar_class('content-area')); ?>">
		...
		<?php if ( have_posts() ) : ?>
			...
			$view   = ( !empty($fablio_theme_options['teamcat_view']) ) ? $fablio_theme_options['teamcat_view'] : 'top-image' ;
			$column = ( !empty($fablio_theme_options['teamcat_column']) ) ? $fablio_theme_options['teamcat_column'] : 'three' ;
