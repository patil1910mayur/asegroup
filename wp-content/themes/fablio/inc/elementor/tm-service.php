...
class Fablio_Servicebox_Widget extends Widget_Base{

	public function get_name() {
		return 'tm_service_element';
	}

	public function get_title() {
		return esc_attr__( 'Service Box', 'fablio' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'style',
			[
				'label'			=> esc_attr__( 'Select Service Style', 'fablio' ),
				'description'	=> esc_attr__( 'Select box design.', 'fablio' ),
				'type' => Controls_Manager::SELECT,
				'label_block'	=> true,
				'options' => [
					'styleone'	=> esc_attr( 'ServiceBox Style 1' ),
					'styletwo'	=> esc_attr( 'ServiceBox Style 2' ),
					'stylethree'	=> esc_attr( 'ServiceBox Style 3' ),
					'stylefour'	=> esc_attr( 'ServiceBox Style 4' ),
				],
		...
		$this->add_control(
			...
			[
				...
				'default' => esc_attr__( 'Our Services', 'fablio' ),
		...
		if ( $posts->have_posts() ) {
			...
			<div class="themetechmount-box-heading-wrapper <?php echo themetechmount_wp_kses($headingclass); ?>">
				...
				if( function_exists('themetechmount_get_filterbutton') ){
					$sortable_category_html = themetechmount_get_filterbutton( $settings, 'tm_service_category' );
			...
			while ( $posts->have_posts() ) {
				...
				if( file_exists( locate_template( '/template-parts/servicebox/servicebox-'.esc_attr($style).'.php', false, false ) ) ){
					...
					ob_start();
					$r = include( locate_template( '/template-parts/servicesbox/servicebox-'.esc_attr($style).'.php', false, false ) );
	...
	protected function select_category() {
	  	$category = get_terms( array( 'taxonomy' => 'tm_service_category', 'hide_empty' => false ) );
	  	$cat = array();
...
// Register widget
Plugin::instance()->widgets_manager->register_widget_type( new Fablio_Servicebox_Widget() );