...
class Fablio_Teambox_Widget extends Widget_Base{

	public function get_name() {
		return 'tm_team_element';
	}
	
	public function get_title() {
		return esc_attr__( 'Team Box', 'fablio' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'style',
			[
				'label'			=> esc_attr__( 'Select Teambox Style', 'fablio' ),
		...
		$this->add_control(
			...
			[
				...
				'default' => esc_attr__( 'Our Team', 'fablio' ),
	...
	protected function render() {
		...
		if ( $posts->have_posts() ) {
			...
			while ( $posts->have_posts() ) {
				$return = '';
				$posts->the_post();
				if( file_exists( locate_template( '/template-parts/teambox/teambox-'.esc_attr($style).'.php', false, false ) ) ){
					...
					ob_start();
					$r = include( locate_template( '/template-parts/teambox/teambox-'.esc_attr($style).'.php', false, false ) );
	...
	protected function select_category() {
	  	$category = get_terms( array( 'taxonomy' => 'tm_team_group', 'hide_empty' => false ) );
	  	$cat = array();
...
// Register widget
Plugin::instance()->widgets_manager->register_widget_type( new Fablio_Teambox_Widget() );