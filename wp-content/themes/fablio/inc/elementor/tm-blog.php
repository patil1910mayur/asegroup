...
class Fablio_Blogbox_Widget extends Widget_Base{

	public function get_name() {
		return 'tm_blog_element';
	}

	public function get_title() {
		return esc_attr__( 'Blog Box', 'fablio' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'style',
			[
				'label'			=> esc_attr__( 'Select Blogbox Style', 'fablio' ),
				'description'	=> esc_attr__( 'Select Blogbox style.', 'fablio' ),
				'type' => Controls_Manager::SELECT,
				'label_block'	=> true,
				'options' => [
					'style1'	=> esc_attr( 'BlogBox Style 1' ),
					'style4'	=> esc_attr( 'BlogBox Style 2' ),
					'style2'	=> esc_attr( 'BlogBox Style 3' ),
					'left-image'=> esc_attr( 'BlogBox Style 4' ),
					'top-image'	=> esc_attr( 'BlogBox Style 5' ),
					'style6'	=> esc_attr( 'BlogBox Style 6' ),
				],
		...
		$this->add_control(
			...
			[
				...
				'default' => esc_attr__( 'Blog', 'fablio' ),
		...
		if ( $posts->have_posts() ) {
			...
			<div class="themetechmount-box-heading-wrapper <?php echo themetechmount_wp_kses($headingclass); ?>">
				...
				if( function_exists('themetechmount_get_filterbutton') ){
					$sortable_category_html = themetechmount_get_filterbutton( $settings, 'category' );
			...
			while ( $posts->have_posts() ) {
				...
				if( file_exists( locate_template( '/template-parts/blogbox/blogbox-'.esc_attr($style).'.php', false, false ) ) ){
					...
					ob_start();
					$r = include( locate_template( '/template-parts/blogbox/blogbox-'.esc_attr($style).'.php', false, false ) );
...
// Register widget
Plugin::instance()->widgets_manager->register_widget_type( new Fablio_Blogbox_Widget() );