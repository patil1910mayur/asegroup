...
function themetechmount_megamenu_override_default_theme($value) {
  ...
  if ( !isset($value['themetechmount-main-menu']['theme']) ) {
    $value['themetechmount-main-menu']['theme'] = 'fablio_theme_settings'; // change my_custom_theme_key to the ID of your exported theme
  }
...
function themetechmount_mmmenu_breakpoint( $options ){
	...
	if( empty($megamenu_themes) ){
		$megamenu_themes = array (
			'fablio_theme_settings' => 
				array (
				'title' => esc_attr__('Fablio Theme Settings', 'fablio'),
				'arrow_up' => 'dash-f343',
...
function themetechmount_image_sizes(){
	...
	add_image_size( 'themetechmount-img-700x770', 700, 770, true ); // For Portfolio
	add_image_size( 'themetechmount-img-blog6', 370, 498, true ); // For Blog
}
}
...
function themetechmount_single_skin_color(){
	...
	if( $post_id ){
		...
		if( !empty($page_meta['skincolor']) ){
			$fablio_theme_options['skincolor'] = esc_attr($page_meta['skincolor']);
		}
	}
}
}
				...
				if( substr($meta, 0, strlen($prefix) ) == $prefix ){
					...
					if( in_array($meta_name, $all_options) ){
						if( themetechmount_is_json($value[0]) && !is_numeric($value[0]) ){
							...
							$original_val = ( isset($fablio_theme_options[$meta_name]) ) ? $fablio_theme_options[$meta_name] : array() ;
							$final_val    = array_merge( $original_val, $final_val );
						} else {
							// string
							$final_val = $value[0];
						}
						$fablio_theme_options[$meta_name] = $final_val;
					}
...
/**
 * Flush out the transients used in {@see fablio_categorized_blog()}.
 *
 * @since Fablio 1.0
 */
if( !function_exists('fablio_category_transient_flusher') ){
function fablio_category_transient_flusher() {
	delete_transient( 'fablio_categories' );
}
}
add_action( 'edit_category', 'fablio_category_transient_flusher' );
add_action( 'save_post',     'fablio_category_transient_flusher' );
...
function themetechmount_add_inline_dynamic_css(){
	...
	include get_template_directory().'/css/dynamic-style2.php';
	if( $fablio_theme_options['advance_themecolors']=='advance_color' ){
		include get_template_directory().'/css/dynamic-style2.php';
	}
	...
	// Minify CSS style
	if( !empty( $fablio_theme_options['minify'] ) && esc_attr($fablio_theme_options['minify'])==true ){
		$css = themetechmount_minify_css( $css );
	}
	
	// fablio main style
	wp_add_inline_style( 'fablio-main-style', $css );
...
function themetechmount_footer_google_fonts( $font_family, $font_weight='normal' ){
	...
	if( !empty($tm_global_footer_gfonts) ){
		...
		wp_enqueue_style(  'themetechmount-footer-gfonts',  '//fonts.googleapis.com/css?family='.$fontline );
...
function themetechmount_single_body_background(){
	...
	if( $post_id ){
		...
		if( isset($single_meta['custom_background_switcher']) && $single_meta['custom_background_switcher']==true && !empty($single_meta['custom_background']) ){
			...
			wp_add_inline_style( 'fablio-last-checkpoint', $css );
		}
...
if ( ! function_exists( 'fablio_excerpt_more' ) && ! is_admin() ) :
/**
 ...
 */
function fablio_excerpt_more( $more ) {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		esc_url( get_permalink( get_the_ID() ) ),
		sprintf( esc_attr__( 'Continue reading %s', 'fablio' ), '<span class="screen-reader-text">' . get_the_title( get_the_ID() ) . '</span>' )
		);
	return ' &hellip; ' . $link;
}
add_filter( 'excerpt_more', 'fablio_excerpt_more' );
endif;
...
function themetechmount_body_classes($bodyClass){
	...
	if( isset($fablio_theme_options['titlebar_view']) && 
		($fablio_theme_options['titlebar_view'] == 'default' || $fablio_theme_options['titlebar_view'] == 'allleft' || $fablio_theme_options['titlebar_view'] == 'allright' ) &&
		isset($fablio_theme_options['breadcrumb_on_bottom']) &&
		$fablio_theme_options['breadcrumb_on_bottom'] == true
	){
		$bodyClass[] = 'tm-titlebar-bcrumb-bottom';
	}
	
	// Check if slider exists for header style classic
	if( !empty($fablio_theme_options['headerstyle']) && $fablio_theme_options['headerstyle']=='classic-box-overlay' && ( is_single() || is_page() ) ){
		...
		if( !empty($singular_meta['slidertype']) && $singular_meta['slidertype']=='revslider' && !empty($singular_meta['revslider']) ){
			$bodyClass[] = 'tm-pageslider-yes';
		}
	}
	
	// check if dark background set for container.
	if( !empty($fablio_theme_options['headerstyle']) ){
		$bodyClass[] = 'tm-headerstyle-'.esc_attr($fablio_theme_options['headerstyle']);
	}
	
	// check if dark background set for container.
	if( isset($fablio_theme_options['inner_background']['background-color']) && trim($fablio_theme_options['inner_background']['background-color'])!='' && themetechmount_check_dark_color(esc_attr($fablio_theme_options['inner_background']['background-color'])) ){
		$bodyClass[] = 'tm-dark-layout';
	}
	
	// show/hide separator line between links in dropdown menu
	if( isset($fablio_theme_options['dropdown_menu_separator']) && trim($fablio_theme_options['dropdown_menu_separator'])=='0' ){
		$bodyClass[] = 'tm-dropmenu-hide-sepline';
	}
	

	// Sticky Fotoer ON/OFF
	if( isset($fablio_theme_options['stickyfooter']) && $fablio_theme_options['stickyfooter']==true ){
		$bodyClass[] = 'themetechmount-sticky-footer';
	}
	
	// Fotoer CTA STyle
	if( isset($fablio_theme_options['footer_cta_styles']) && $fablio_theme_options['footer_cta_styles']== 'style1' ){
		$bodyClass[] = 'themetechmount-cta-style1';
	}
	
	// Single Portfolio
	if( is_singular('portfolio') ){
		$portfolioView        = esc_attr($fablio_theme_options['portfolio_viewstyle']); // Global view
	...
	if( isset($fablio_theme_options['show_topbar']) && $fablio_theme_options['show_topbar']==true ){
		$bodyClass[] = sanitize_html_class('themetechmount-topbar-yes');
	} else {
		$bodyClass[] = sanitize_html_class('themetechmount-topbar-no');
	}
	
	if( isset($fablio_theme_options['footer_cta_box']) && $fablio_theme_options['footer_cta_box']==true ){
		$bodyClass[] = sanitize_html_class('themetechmount-footer-cta-yes');
	}
	
	// Boxed / Wide
	if( isset($fablio_theme_options['layout']) && trim($fablio_theme_options['layout'])!='' ){
		if( $fablio_theme_options['layout']=='boxed' || $fablio_theme_options['layout']=='framed' || $fablio_theme_options['layout']=='rounded' ){
			$bodyClass[] = 'themetechmount-boxed';
		}
		if( $fablio_theme_options['layout']=='framed' || $fablio_theme_options['layout']=='rounded' ){
			$bodyClass[] = 'themetechmount-boxed-'.sanitize_html_class($fablio_theme_options['layout']);
		}
		
		$bodyClass[] = sanitize_html_class( 'themetechmount-'.trim($fablio_theme_options['layout']));
		if( $fablio_theme_options['layout']=='fullwide' ){
			if( isset($fablio_theme_options['full_wide_elements']['content']) && $fablio_theme_options['full_wide_elements']['content']=='1' ){
	...
	if( isset($fablio_theme_options['logoimg_retina']['url']) && $fablio_theme_options['logoimg_retina']['url']!=''){
	...
	$headerstyle_global = '';
	$headerstyle_page   = '';
	if( isset($fablio_theme_options['headerstyle']) && trim($fablio_theme_options['headerstyle'])!='' ){
		$headerstyle_global = sanitize_html_class($fablio_theme_options['headerstyle']);
	}
	if( is_page() ){
	...
	// One Page website
	if( isset($fablio_theme_options['one_page_site']) && $fablio_theme_options['one_page_site']==true ){
...
add_filter( 'admin_body_class', 'themetechmount_newadmin_body_class' );
function themetechmount_newadmin_body_class( $classes ) {
	
	if ( defined( 'ELEMENTOR_PATH' ) && class_exists('Elementor\Widget_Base') ) {	
		return "$classes tm-elementor-default";
	}
	else {
		return "$classes tm-wpbakery-default";
	}
}