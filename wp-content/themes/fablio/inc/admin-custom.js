...
function themetechmount_header_style_change_triggers(){
	...
	var topbar_right_text			  = '[tm-social-links tooltip="no"]<div class="tm-vc_btn3-container tm-vc_btn3-inline"><a class="tm-vc_general tm-vc_btn3 tm-vc_btn3-size-md tm-vc_btn3-shape-square tm-vc_btn3-style-flat tm-vc_btn3-weight-no tm-vc_btn3-color-skincolor" href="#" title="">Get An Appointment!</a></div>';
	...
	var header_text      			  = '<a class="tm-vc_general tm-vc_btn3 tm-vc_btn3-size-md tm-vc_btn3-shape-rounded tm-vc_btn3-style-flat tm-vc_btn3-weight-no tm-vc_btn3-color-gradient" href="#" title="">Get A Quote</a>';
	var infostack_column_one		  = '<div class="header-icon"> <div class="icon"><i class="kw_fablio flaticon-phone-call"></i></div></div><div class="header-content"><h3>Telephone</h3><h5>+123 795 9841</h5></div>';
	var infostack_column_two		  = '<div class="header-icon"> <div class="icon"><i class="kw_fablio flaticon-big-envelope"></i></div></div><div class="header-content"><h3>Email Address</h3><h5>info@example.com</h5></div>';
	var infostack_column_three		  = '<div class="header-icon"> <div class="icon"><i class="kw_fablio flaticon-pin"></i></div></div><div class="header-content"><h3>Office Address</h3><h5> 23 Belfast ave, Florida</h5></div>';
	
	jQuery('input[name="fablio_theme_options[headerstyle]"]').change(function() {
		...
		if (this.value == 'classic') { // Classic
			jQuery('select[name="fablio_theme_options[header_bg_color]"]').val(header_bg_color).change();
			jQuery('input[name="fablio_theme_options[titlebar_background][color]"]').iris('color', titlebar_background_color);  // 
			jQuery('select[name="fablio_theme_options[titlebar_text_color]"]').val(titlebar_text_color);
			jQuery('input[name="fablio_theme_options[titlebar_height]"').val(titlebar_height);  // Titlebar height
			jQuery('input[name="fablio_theme_options[show_topbar]"]').prop('checked', true);  // Show topbar
			jQuery('input[name="fablio_theme_options[header_search]"]').prop('checked', false);  // Show header search
			jQuery('input[name="fablio_theme_options[mainmenufont][color]"').iris('color', mainmenufont_color);  // 
			jQuery('input[name="fablio_theme_options[header_height]"]').val(header_height);  // Header Height
			jQuery('input[name="fablio_theme_options[logoimg][id]"]').val('');
			jQuery('input[name="fablio_theme_options[logoimg][thumb-url]"]').val( imgurl+logoimg );
			jQuery('input[name="fablio_theme_options[logoimg][full-url]').val( imgurl+logoimg );
			jQuery('input[name="fablio_theme_options[logo_max_height]"]').val(logo_max_height);  //
			jQuery('select[name="fablio_theme_options[mainmenu_active_link_color]"]').val(mainmenu_active_link_color).change(); // Select
			jQuery('input[name="fablio_theme_options[stickymainmenufontcolor]"]').iris('color', stickymainmenufontcolor_color);  // 
			jQuery('select[name="fablio_theme_options[sticky_header_bg_color]"]').val(sticky_header_bg_color).change();		
			jQuery('input[name="fablio_theme_options[dropmenu_background][color]"]').iris('color', dropmenu_background_color);  // Dropdown Menu color
			jQuery('input[name="fablio_theme_options[dropdownmenufont][color]"]').iris('color', dropdownmenufont_color);  // Dropdown Menu color
			jQuery('textarea[name="fablio_theme_options[header_text]"]').val(header_text);  //
...
function themetechmount_show_hide_titlebar_textcolor(){
	...
	var $this      = jQuery( 'select[name="fablio_theme_options[titlebar_text_color]"]' );
	var $page_this = jQuery( 'select[name="_themetechmount_metabox_group[titlebar_text_color]"]' );
	
	if( $this.length > 0 ){
		if( jQuery($this).val()=='custom' ){
			jQuery( 'input[name="fablio_theme_options[titlebar_heading_font][color]"]' ).closest('.tm-typography-font-color-w').show();
			jQuery( 'input[name="fablio_theme_options[titlebar_subheading_font][color]"]' ).closest('.tm-typography-font-color-w').show();
			jQuery( 'input[name="fablio_theme_options[titlebar_breadcrumb_font][color]"]' ).closest('.tm-typography-font-color-w').show();
		} else {
			jQuery( 'input[name="fablio_theme_options[titlebar_heading_font][color]"]' ).closest('.tm-typography-font-color-w').hide();
			jQuery( 'input[name="fablio_theme_options[titlebar_subheading_font][color]"]' ).closest('.tm-typography-font-color-w').hide();
			jQuery( 'input[name="fablio_theme_options[titlebar_breadcrumb_font][color]"]' ).closest('.tm-typography-font-color-w').hide();
		}
	}
...
function themetechmount_show_hide_color_picker_background(){
	...
	jQuery(items).each(function(n,val){
		...
		var $dropdown_element   = jQuery( 'select[name="fablio_theme_options['+dropdown_name+']"]' );
		
		// show/hide the color picker on load
		if( $dropdown_element.val()=='custom' ){
			jQuery( 'input[name="fablio_theme_options['+background_name+'][color]"]' ).closest('.tm-background-color-w').show();
		} else {
			jQuery( 'input[name="fablio_theme_options['+background_name+'][color]"]' ).closest('.tm-background-color-w').hide();
		}
...
jQuery(document).ready( function() {
	...
	jQuery('.cs-field-themetechmount_background', this).THEMETECHMOUNT_CSFRAMEWORK_BG_IMAGE_UPLOADER();
	jQuery('.cs-field-themetechmount_typography', this).THEMETECHMOUNT_CSFRAMEWORK_TYPOGRAPHY();
	jQuery('.cs-field-themetechmount_image', this).THEMETECHMOUNT_CSFRAMEWORK_IMAGE_UPLOADER();
	
	
	
	// Titlebar Text Color - Show / Hide color for Text color option
	themetechmount_show_hide_titlebar_textcolor();
	jQuery( 'select[name="fablio_theme_options[titlebar_text_color]"]' ).change(function() {  // Theme Options
		themetechmount_show_hide_titlebar_textcolor();
	});
	...
	jQuery('input[type=radio][name="fablio_theme_options[loaderimg]"]:checked').closest('label').addClass('tm-pre-loader-option-selected');
	jQuery('input[type=radio][name="fablio_theme_options[loaderimg]"]').change(function() {
		jQuery('input[type=radio][name="fablio_theme_options[loaderimg]"]').closest('label').removeClass('tm-pre-loader-option-selected');
