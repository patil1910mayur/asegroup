 ...
 */
if( ! function_exists( 'fablio_cs_framework_init' ) ) {
function fablio_cs_framework_init(){
	// Codestar framework config
	defined('CS_OPTION'          ) or define('CS_OPTION',           'fablio_theme_options' );
...
add_action( 'init', 'fablio_cs_framework_init', 1 );
add_action( 'admin_init', 'fablio_cs_framework_init', 1 );
			...
			if( isset($section['fields']) && is_array($section['fields']) ){
				foreach( $section['fields'] as $field ){
					if( isset($field['id']) && isset($field['default']) ){
						...
						$fablio_theme_options[$field_id] = $field_val;
					}
				}
...
function themetechmount_get_font_option_values($elements){
	...
	if( is_array($elements) && count($elements)>0 ){
		...
		foreach( $elements as $element ){
			$val = ( isset($fablio_theme_options[$element]) ) ? $fablio_theme_options[$element] : array() ;
