...
final class UserFeedback_Lite extends UserFeedback_Base {
	...
	public function add_vue_notices( $notices ) {
		...
		if( 'install-complete' !== $onboarding_step ){
			$onboarding_url = admin_url('admin.php?page=userfeedback_onboarding');
			$notices[]    = array(
				'id'      => 'onboarding-prompt',
				'content' => sprintf(__('%sClick here%s to complete userfeedback setup.', 'userfeedback'), '<a rel="noopener" href="'.$onboarding_url.'">', '</a>')
			);
		}
