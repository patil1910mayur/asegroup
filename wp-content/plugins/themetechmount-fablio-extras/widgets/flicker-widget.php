 ...
 * @since 1.0
 */
class fablio_flicker_widget extends WP_Widget {
	...
	function __construct() {
		$widget_style = array('classname'   => 'fablio_flicker_widget',
							  'description' => esc_attr__('Show photos from Flickr.','fablio') );
		...
		parent::__construct(
			'fablio_flicker_widget', // Base ID
			esc_attr__('Fablio Flickr Widget', 'fablio'), // Name
			$widget_style // Args
		);
	}
	...
	function widget( $args, $cur_instance ) {
		...
		echo '<div class="fablio_flicker_widget_wrapper">'; ?>
	...
	function form( $cur_instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php echo esc_attr__('Title', 'fablio'); ?>:</label>
...
register_widget( 'fablio_flicker_widget' );