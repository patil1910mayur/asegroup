...
function themetechmount_fablio_cpt_tm_testimonial(){
	...
	function themetechmount_fablio_tm_testimonial_enter_title_here( $title ){
	...
	add_filter( 'enter_title_here', 'themetechmount_fablio_tm_testimonial_enter_title_here' );


	// Move Featured Image box from left to center only on CLIENTS custom_post_type
	add_action('do_meta_boxes', 'themetechmount_fablio_tm_testimonial_featured_image_box');
	function themetechmount_fablio_tm_testimonial_featured_image_box() {
...
add_action( 'init', 'themetechmount_fablio_cpt_tm_testimonial', 8 );
 ...
 *  Meta Box: Clients
 */
if ( ! function_exists( 'themetechmount_fablio_tm_testimonials_metabox_options' ) ) {
function themetechmount_fablio_tm_testimonials_metabox_options( $options ) {
	...
	$options[]    = array(
		...
		'title'     => esc_attr__('Fablio: Testimonial Details', 'tmte'),
			...
			array(
				...
				'fields' => array(
					...
					array(
						'id'     		=> 'highlighttext',
						'type'    		=> 'text',
						'title'   		=> esc_attr__('Testimonial High Light Text', 'tmte'),
						'after'  		=> '<div class="cs-text-muted"><br>'.__("(Optional) Please fill highlight text for testimonial", 'tmte').'</div>',
					),
				),
			),
		),
...
add_filter( 'cs_metabox_options', 'themetechmount_fablio_tm_testimonials_metabox_options' );
