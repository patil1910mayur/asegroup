...
function themetechmount_fablio_cpt_tm_portfolio(){

	$fablio_theme_options = get_option('fablio_theme_options');
	
	$pf_type_title          = ( !empty($fablio_theme_options['pf_type_title']) ) ? $fablio_theme_options['pf_type_title'] : 'Portfolio' ;
	...
	$pf_type_slug           = ( !empty($fablio_theme_options['pf_type_slug']) ) ? $fablio_theme_options['pf_type_slug'] : 'portfolio' ;
	
	$pf_cat_title          = ( !empty($fablio_theme_options['pf_cat_title']) ) ? $fablio_theme_options['pf_cat_title'] : 'Portfolio Categories' ;
	$pf_cat_title_singular = ( !empty($fablio_theme_options['pf_cat_title_singular']) ) ? $fablio_theme_options['pf_cat_title_singular'] : 'Portfolio Category' ;
	$pf_cat_slug           = ( !empty($fablio_theme_options['pf_cat_slug']) ) ? $fablio_theme_options['pf_cat_slug'] : 'portfolio-category' ;
...
add_action( 'init', 'themetechmount_fablio_cpt_tm_portfolio', 8 );
 ...
 */
if ( ! function_exists( 'themetechmount_fablio_tm_portfolio_metabox_options' ) ) {
function themetechmount_fablio_tm_portfolio_metabox_options( $options ) {
	...
	$list_array    = array();
	$options_array = array();
	if( isset($fablio_theme_options['pf_details_line']) && is_array($fablio_theme_options['pf_details_line']) && count( $fablio_theme_options['pf_details_line'] ) > 0 ){
		foreach( $fablio_theme_options['pf_details_line'] as $key=>$val ){
			...
			$option_array = array(
				...
				'type'       => 'text',
				'title'      => '<span class="tm-admin-pf-list-icon"> <i class="'. $icon_class .'"></i></span> &nbsp; '. esc_attr__($val['pf_details_line_title'], 'fablio'),
			);
	...
	$options[]    = array(
		...
		'title'     => sprintf( esc_attr__('Fablio: %s List Options', 'tmte'), $pf_type_title_singular ),
		...
		'sections'  => array(
			array(
				...
				'fields' => array(
					...
					array(
						"id"    => "project_details_line_btn_text",
						"type"  => "text",
						"title" => '<span class="tm-admin-team-list-icon"></span> &nbsp; '.__('View Project Button Text', 'tmte'),
					),
					array(
						"id"    => "project_details_line_btn_link",
						"type"  => "text",
						"title" => '<span class="tm-admin-team-list-icon"></span> &nbsp; '.__('View Project Button Link', 'tmte'),
					),	
				),
			),
		),
	...
	$options[]    = array(
		...
		'title'     => sprintf( esc_attr__('Fablio: %s View Style', 'tmte'), $pf_type_title_singular ),
		'post_type' => 'tm_portfolio', // only here is important
		'context'   => 'normal',
		'priority'  => 'default',
		'sections'  => array(
				...
				'fields' => array(
					...
					array(
						...
						'options'       => array(
									...
									'left-style2' 	=> esc_attr__('Left image and right content Style2', 'tmte'),
	...
	$options[]    = array(
		...
		'title'     => esc_attr__('Fablio: Portfolio Like Option','tmte'),
		'post_type' => 'tm_portfolio', // only here is important
		'context'   => 'normal',
		'priority'  => 'default',
		'sections'  => array(
...
add_filter( 'cs_metabox_options', 'themetechmount_fablio_tm_portfolio_metabox_options' );