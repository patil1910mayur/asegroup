...
function themetechmount_fablio_cpt_tm_team(){
	...
	$team_type_slug           = ( !empty($fablio_theme_options['team_type_slug']) ) ? $fablio_theme_options['team_type_slug'] : 'team-member' ;
	...
	$team_cat_slug           = ( !empty($fablio_theme_options['team_cat_slug']) ) ? $fablio_theme_options['team_cat_slug'] : 'team-group' ;
	...
	add_action('do_meta_boxes', 'themetechmount_fablio_tm_team_featured_image_box');
	function themetechmount_fablio_tm_team_featured_image_box() {
...
add_action( 'init', 'themetechmount_fablio_cpt_tm_team', 8 );
 ...
 *  Meta Box: Team
 */
if ( ! function_exists( 'themetechmounttmte_fablio_tm_team_metabox_options' ) ) {
function themetechmounttmte_fablio_tm_team_metabox_options( $options ) {
	...
	$team_extra_details_lines  = ( !empty($fablio_theme_options['team_extra_details_lines']) ) ? $fablio_theme_options['team_extra_details_lines'] : array() ;
	...
	if( is_array($team_extra_details_lines) && count($team_extra_details_lines) > 0 ){
		...
		foreach( $fablio_theme_options['team_extra_details_lines'] as $key=>$val ){
			...
			$this_array['title'] = '<span class="tm-admin-team-list-icon"> <i class="'. $icon_class .'"></i></span> &nbsp; '. esc_attr__($val['team_extra_details_line_title'], 'fablio');
	...
	$options[]    = array(
		...
		'title'     => sprintf( esc_attr__("Fablio: %s's Details", 'tmte'), $team_type_title_singular ),
	...
	$options[]    = array(
		...
		'title'     => sprintf( esc_attr__("Fablio: %s's Social Links", 'tmte'), $team_type_title_singular ),
		'post_type' => 'tm_team_member', // only here is important
		'context'   => 'normal',
		'priority'  => 'default',
		'sections'  => array(
			...
			array(
				...
				'fields' => array(
					array(
						...
						'info'            => esc_attr__('Add your social services here. Also you can reorder the Social Links as per your choice. Just drag and drop items to reorder and save settings', 'fablio'),
						'button_title'    => esc_attr__('Add New Social Links', 'fablio'),
						'accordion_title' => 'social_icons_list_icon',
						'fields'          => array(
							array(
								'id'            => 'social_icons_list_icon',
								'type'          => 'select',
								'title'         =>  esc_attr__('Social Sevice', 'fablio'),
								'options'  		=> array(
													...
													'rss'        => esc_attr__('RSS', 'fablio' ),
												),
								'class'         => 'chosen',
								'default'       => 'twitter',
								'after'  		=> '<div class="cs-text-muted"><br>'.__('Select Social service from here', 'fablio').'</div>',
							),
							array(
								'id'     		=> 'social_icons_list_link',
								'type'    		=> 'text',
								'title'   		=> esc_attr__('Link to Social service selected above', 'fablio'),
...
add_filter( 'cs_metabox_options', 'themetechmounttmte_fablio_tm_team_metabox_options' );
