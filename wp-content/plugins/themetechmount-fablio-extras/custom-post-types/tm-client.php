...
function themetechmount_fablio_cpt_tm_client(){
	...
	function themetechmount_fablio_tm_client_enter_title_here( $title ){
	...
	add_filter( 'enter_title_here', 'themetechmount_fablio_tm_client_enter_title_here' );




	// Move Featured Image box from left to center only on CLIENTS custom_post_type
	add_action('do_meta_boxes', 'themetechmount_fablio_tm_client_featured_image_box');
	function themetechmount_fablio_tm_client_featured_image_box() {
...
add_action( 'init', 'themetechmount_fablio_cpt_tm_client', 8 );
 ...
 *  Meta Box: Clients
 */
if ( ! function_exists( 'themetechmount_fablio_tm_client_metabox_options' ) ) {
function themetechmount_fablio_tm_client_metabox_options( $options ) {
	...
	$options[]    = array(
		...
		'title'     => esc_attr__('Fablio: Client Details', 'tmte'),
...
if( function_exists('cs_framework_init') ){
	add_filter( 'cs_metabox_options', 'themetechmount_fablio_tm_client_metabox_options' );
}