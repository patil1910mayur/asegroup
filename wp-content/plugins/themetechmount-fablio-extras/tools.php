...
function themetechmount_pagination( $wp_query_data=false ){
	...
	$paginateLinks = paginate_links( array(
		...
		'type'      => 'array',
		'prev_text' => '<i class="tm-fablio-icon-arrow-left"></i> <span class="tm-hide tm-pagination-text tm-pagination-text-prev">' . esc_attr__( 'Previous page', 'fablio' ) . '</span>',
		'next_text' => '<span class="tm-hide tm-pagination-text tm-pagination-text-next">' . esc_attr__( 'Next page', 'fablio' ) . '</span> <i class="tm-fablio-icon-arrow-right"></i>',
	) );
			...
			switch( $meta_id ){
				...
				case 'date':
					...
					if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
						...
						$return .= sprintf( '<span class="tm-meta-line posted-on"><i class="tm-fablio-icon-clock"></i> <span class="screen-reader-text tm-hide">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
				...
				case 'comment':
					...
					if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
						$return .= '<span class="tm-meta-line comments-link"><i class="tm-fablio-icon-comments-smiley"></i> ';
						ob_start();
						comments_popup_link( esc_attr__( 'Leave a comment', 'fablio' ) );
				...
				case 'tag':
					...
					if ( !empty($tags_list) ) {
						...
						$return .= sprintf( '<span class="tm-meta-line tags-links"><i class="tm-fablio-icon-tag"></i> <span class="screen-reader-text tm-hide" tm-hide>%1$s </span>%2$s</span>',
	...
	if( !empty($type) ){
		...
		if( !empty($return) ){
			...
			switch( $type ){
				...
				case 'phone':
					$return = '<div class="tm-team-list-title tm-skincolor"><i class="tm-fablio-icon-phone"></i> '. esc_attr__('Phone','fablio') .'</div>
						<div class="tm-team-list-value"><a href="tel:' . $return . '">' . $return . '</a></div>';
					break;
					
				case 'email':
					$return = '<div class="tm-team-list-title tm-skincolor"><i class="tm-fablio-icon-mail"></i> '. esc_attr__('Email','fablio') .'</div>
				...
				case 'website':
					...
					$return = '<div class="tm-team-list-title tm-skincolor"><i class="tm-fablio-icon-world"></i> '. esc_attr__('Website','fablio') .'</div>
...
function themetechmount_portfolio_next_prev_btn() {
	...
	if( is_singular('tm_portfolio') ){
		...
		$return = get_the_post_navigation( array(
			'next_text' => '<span class="meta-nav tm-hide" aria-hidden="true">' . __( 'Next', 'fablio' ) . '</span> ' .
				'<span class="screen-reader-text tm-hide">' . __( 'Next post:', 'fablio' ) . '</span> ' .
				'<span class="post-title tm-hide">%title</span>',
			'prev_text' => '<span class="meta-nav tm-hide" aria-hidden="true">' . __( 'Previous', 'fablio' ) . '</span> ' .
				'<span class="screen-reader-text tm-hide">' . __( 'Previous post:', 'fablio' ) . '</span> ' .
				'<span class="post-title tm-hide">%title</span>',
		) );
...
function themetechmount_portfolio_detailsbox(){
	...
	if( isset($pf_details_line) && is_array($pf_details_line) && count($pf_details_line)>0 ){
		foreach( $pf_details_line as $key=>$val ){
			...
			if( !empty($val['pf_details_line_title']) ){ $row_title = sprintf( __('%s', 'fablio'), $val['pf_details_line_title'] ); }
...
function themetechmount_portfoliobox_media_link( $default_icon='tm-fablio-icon-search', $video_icon='tm-fablio-icon-video', $audio_icon='tm-fablio-icon-music-alt', $slider_icon='tm-fablio-icon-gallery-1' ){
	...
	$icon_code='<i class="tm-fablio-icon-search"></i>';
...
function themetechmount_global_portfolio_template_list( $for_vc=false ){
	$return = array(
		'overlay'    => esc_attr__('Content overlay on image view', 'fablio'),
		'top-image'  => esc_attr__('Top Image and Bottom Content view', 'fablio'),
	);
...
function themetechmount_global_blog_template_list( $for_vc=false ){
	$return = array(
		"top-image"       => esc_attr__("Top image and bottom content (default)", "fablio"),
		"left-image"      => esc_attr__('Left image and right content ', "fablio"),
		"right-image"     => esc_attr__('Right image and left content ', "fablio"),
		"content-overlay" => esc_attr__('Content overlay on image', "fablio"),
	);
...
function themetechmount_global_team_member_template_list( $for_vc=false ){
	$return = array(
		"overlay"    => esc_attr__("Overlay content on Image (Default)", "fablio"),
		"left-image" => esc_attr__("Left Image and Right Content", "fablio"),
	);
...
function themetechmount_global_client_template_list( $for_vc=false ){
	$return = array(
		'simple-logo'    => esc_attr__('Simple Logo view', 'fablio'),
		'separator-logo' => esc_attr__('Logo with Separator view', 'fablio'),
		'boxed-logo'     => esc_attr__('Boxed view', 'fablio'),
		//'logo-with-title' => esc_attr__('Logo with Title view', 'fablio'),
	);
...
function themetechmount_header_links(){
	...
	if( function_exists('is_woocommerce') && $wc_header_icon==true ){
		...
		$return  .= '<span class="tm-header-icon tm-header-wc-cart-link"><a href="' . $cart_url . '"><i class="tm-fablio-icon-cart"></i><span class="number-cart">&nbsp;</span></a></span>';
	}
	...
	if( $header_search==true ){
		$class   = '';
		$return .= '<span class="tm-header-icon tm-header-search-link"><a href="javascript:void(0);"><i class="tm-fablio-icon-search"></i></a></span>';
	}
...
function themetechmount_testimonial_featured_image($size='thumbnail'){
	...
	$return = ( !empty($featured_image) ) ? $featured_image : '<span class="themetechmount-icon-box"><i class="demo-icon tm-fablio-icon-quote-left"></i></span>';
...
function themetechmount_event_description(){ 
	$return   = '';
	$readMore = esc_attr__('See Event', 'fablio') . ' <i class="kwicon-fa-angle-right"></i>';
...
function themetechmount_comment_row_template($comment, $args, $depth){
    ...
    <div class="comment-meta commentmetadata">
		<?php printf( esc_attr__( '<cite class="tm-comment-owner fn">%s</cite> <span class="says">says:</span>', 'fablio' ), get_comment_author_link() ); ?>
...
function themetechmount_search_results_box_title( $post_type='post' ){
	...
	if( !empty($singular_name) ){
		...
		$return .= '<div class="tm-sresults-title-w"><h2 class="tm-sresults-title">' . sprintf(
				esc_attr__('Search results for %s','fablio'),
